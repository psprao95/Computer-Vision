...
# function to restrict linear and non-linear RGB values to (0,1) range
def limit(b,g,r):
    input=[b,g,r]
    output=[]
    for i in input:
...
# function to restrict sRGB values to (0,255) range
def limitRGB(b,g,r):
    input=[b,g,r]
    output=[]
    for i in input:
...
# function to convert from sRGB to non-linear RGB
def sRGB_To_NonLinearRGB(b,g,r):
    return limit(b/255,g/255,r/255)

# function to convert from non-linear RGB to linear RGB
...
# function to convert from linear RGB to XYZ
...
# function to convert from XYZ to Luv
...
# function to convert from Luv to  XYZ
...
# function to convert from linear RGB to non-linear RGB
...
# function to convert from non-linear RGB to sRGB
...
# Converting for pixels in given window to Luv domain
for i in range (H1,H2+1):
    for j in range (W1,W2+1):
        ...
        x,y,z=temp[i,j]
        temp[i,j]=XYZ_To_Luv(x,y,z)


# Performing Linear Scaling of L values
Lmin=100
Lmax=0
for i in range (H1,H2+1):
    for j in range (W1,W2+1):
...
# Scaling L values of the pixels in window
for i in range (H1,H2+1):
    for j in range (W1,W2+1):
        L,u,v=temp[i,j]
        Lscaled=(L-Lmin)*100/(Lmax-Lmin)
        temp[i,j]=[Lscaled,u,v]


# converting back to sRGB domain
for i in range (H1,H2+1):
    for j in range (W1,W2+1):
        L,u,v=temp[i,j]
